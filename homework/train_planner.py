...
from homework.datasets import SuperTuxDataset
...
def train(
    ...
    model = load_model(model_name, hidden_dim=hidden_dim, dropout=dropout, num_layers=num_layers)
    ...
    for epoch in range(num_epoch):
        ...
        for img, label in train_data:
            ...
            # calculate accuracy for batch and save to metrics
            batch_acc = (pred_label.argmax(dim=1) == label).float().mean()
            metrics["train_acc"].append(batch_acc.item())
        ...
        with torch.inference_mode():
            ...
            for img, label in val_data:
                ...
                # calculate accuracy for batch and save to metrics
                batch_acc = (pred_label.argmax(dim=1) == label).float().mean()
                metrics["val_acc"].append(batch_acc.item())
